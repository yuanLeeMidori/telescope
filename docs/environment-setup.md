...
There are different ways to run the application. By default, `.env` will be read; however, if there is no `.env` file in the root, [env.development](../config/env.development) will be used.
...
Back-end will start downloading feeds and processing them until stopped. The default port # for the back-end is `3000` and can be modified with the `PORT` variable in the `.env` file. Different routes such as `http://localhost:3000/posts` and
`http://localhost:3000/feeds` now return data. And front-end can be found in `http://localhost:8000` by default.
